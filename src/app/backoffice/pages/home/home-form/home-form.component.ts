...
export class HomeFormComponent{
  ...
  updateSlides(){
    ...
    if(this.previousSlide1 != null && this.currentSlide1 != null){
      this.serviceSlide.updateOrderOfSlide(this.previousSlide1).subscribe(
        response => { response },
      );
      this.serviceSlide.updateOrderOfSlide(this.currentSlide1).subscribe(
        response => { response },
      );
    }
    
    if(this.previousSlide2 != null && this.currentSlide2 != null){
      this.serviceSlide.updateOrderOfSlide(this.previousSlide2).subscribe(
        response => { response },
      );
      this.serviceSlide.updateOrderOfSlide(this.currentSlide2).subscribe(
        response => { response },
      );
    }
    
    if(this.previousSlide3 != null && this.currentSlide3 != null){
      this.serviceSlide.updateOrderOfSlide(this.previousSlide3).subscribe(
        response => { response },
      );
      this.serviceSlide.updateOrderOfSlide(this.currentSlide3).subscribe(
        response => { response },
      );
    }
